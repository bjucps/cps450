...
public class CCodeGenerator {
    ...
    public void traverse(ProgramContext program) {
		...
		// First determine what variables are used
		List<String> variables = new ArrayList<String>();
        for (var stmt : stmts) {
            if (stmt instanceof AsmtStmtContext) {
                String id = (((AsmtStmtContext)stmt).assign_stmt()).ID().getText();
		...
		// Next generate variable definitions
		for (String variable: variables) {
			System.out.println("  int " + variable + ";");
		}
        
		// Now we're ready to translate the body of the program
        for (var stmt : stmts) {
            if (stmt instanceof AsmtStmtContext) {
		...
		// Don't forget to end the main() function
